...
function createCompatibleTranslator(fetchImpl, endpoint) {
  return async (text, targetLang) => {
    const url = new URL(endpoint || DEFAULT_COMPATIBLE_ENDPOINT);
...
function createGoogleCloudTranslator(fetchImpl, apiKey, endpoint) {
  ...
  return async (text, targetLang) => {
    const url = new URL(endpoint || DEFAULT_GOOGLE_CLOUD_ENDPOINT);
...
function createTranslator({
  provider = process.env.OSLT_TRANSLATOR || 'google-compatible',
